...
import no.ssb.lds.neo4j.util.Util;
...
public class ExpandPathTest {
    ...
    public void testExplorePathAnyRelTypeTest() throws Throwable {
        TestUtil.testCall(db,
                "MATCH (m:Movie {title: 'The Matrix'}) CALL lds.path.expand(m,'>','',0,2,datetime()) yield path return count(*) as c",
        ...
        TestUtil.testCall(db,
                "MATCH (m:Movie {title: 'The Matrix'}) CALL lds.path.expand(m,null,'',0,2,datetime()) yield path return count(*) as c",
                (row) -> assertEquals(52L, row.get("c")));
    }

    @Test
    public void testExplorePathRelationshipsTest() throws Throwable {
        String query = "MATCH (m:Movie {title: 'The Matrix'}) CALL lds.path.expand(m,'<ACTED_IN|PRODUCED>|FOLLOWS','',0,2,datetime()) yield path return count(*) as c";
        TestUtil.testCall(db, query, (row) -> assertEquals(11L, row.get("c")));
    }

    @Test
    public void testExplorePathLabelWhiteListTest() throws Throwable {
        String query = "MATCH (m:Movie {title: 'The Matrix'}) CALL lds.path.expand(m,'ACTED_IN|PRODUCED|FOLLOWS','+Person|+Movie',0,3,datetime()) yield path return count(*) as c";
    ...
    public void testExplorePathLabelBlackListTest() throws Throwable {
        String query = "MATCH (m:Movie {title: 'The Matrix'}) CALL lds.path.expand(m,null,'-BigBrother',0,2,datetime()) yield path return count(*) as c";
    ...
    public void testExplorePathWithTerminationLabel() {
        ...
        TestUtil.testResult(db,
                "MATCH (k:Person {name:'Keanu Reeves'}) " +
                        "CALL lds.path.expandConfig(k, {relationshipFilter:'ACTED_IN|PRODUCED|DIRECTED', labelFilter:'/Western', uniqueness: 'NODE_GLOBAL'}, datetime()) yield path " +
                        "return path",
                result -> {
    ...
    public void testExplorePathWithFilterStartNodeFalseIgnoresLabelFilter() throws Throwable {
        String query = "MATCH (m:Movie {title: 'The Matrix'}) CALL lds.path.expandConfig(m,{labelFilter:'+Person', maxLevel:2, filterStartNode:false}, datetime()) yield path return count(*) as c";
    ...
    public void testExplorePathWithLimitReturnsLimitedResults() {
        ...
        TestUtil.testResult(db,
                "MATCH (k:Person {name:'Keanu Reeves'}) " +
                        "CALL lds.path.expandConfig(k, {relationshipFilter:'ACTED_IN|PRODUCED|DIRECTED', labelFilter:'/Western', uniqueness: 'NODE_GLOBAL', limit: 2}, datetime()) yield path " +
    ...
    public void testBlacklistBeforeWhitelist() {
        ...
        TestUtil.testResult(db,
                "MATCH (k:Person {name:'Keanu Reeves'}) " +
                        "CALL lds.path.expandConfig(k, {relationshipFilter:'ACTED_IN|PRODUCED|DIRECTED', labelFilter:'+Person|-Person', uniqueness: 'NODE_GLOBAL', filterStartNode:true}, datetime()) yield path " +
                        "return path",
                result -> {
                    List<Map<String, Object>> maps = Iterators.asList(result);
    ...
    public void testBlacklistBeforeTerminationFilter() {
        ...
        TestUtil.testResult(db,
                "MATCH (k:Person {name:'Keanu Reeves'}) " +
                        "CALL lds.path.expandConfig(k, {relationshipFilter:'ACTED_IN|PRODUCED|DIRECTED', labelFilter:'/Western|-Western', uniqueness: 'NODE_GLOBAL', filterStartNode:false}, datetime()) yield path " +
                        "return path",
                result -> {
                    List<Map<String, Object>> maps = Iterators.asList(result);
                    assertEquals(0, maps.size());
    ...
    public void testTerminationFilterBeforeWhitelist() {
        ...
        TestUtil.testResult(db,
                "MATCH (k:Person {name:'Keanu Reeves'}) " +
                        "CALL lds.path.expandConfig(k, {relationshipFilter:'ACTED_IN|PRODUCED|DIRECTED', labelFilter:'/Western|+Movie', uniqueness: 'NODE_GLOBAL', filterStartNode:false}, datetime()) yield path " +
                        "return path",
                result -> {
                    List<Map<String, Object>> maps = Iterators.asList(result);
    ...
    public void testLimitPlaysNiceWithMinLevel() {
        ...
        TestUtil.testResult(db,
                "MATCH (k:Person {name:'Keanu Reeves'}) " +
                        "CALL lds.path.expandConfig(k, {relationshipFilter:'ACTED_IN|PRODUCED|DIRECTED', labelFilter:'>Western', uniqueness: 'NODE_GLOBAL', limit:1, minLevel:3}, datetime()) yield path " +
                        "return path",
                result -> {
                    List<Map<String, Object>> maps = Iterators.asList(result);
                    assertEquals(1, maps.size());
    ...
    public void testTerminationFilterDoesNotPruneBelowMinLevel() {
        ...
        TestUtil.testResult(db,
                "MATCH (k:Person {name:'Keanu Reeves'}) " +
                        "CALL lds.path.expandConfig(k, {relationshipFilter:'ACTED_IN|PRODUCED|DIRECTED', labelFilter:'/Western', uniqueness: 'NODE_GLOBAL', minLevel:3}, datetime()) yield path " +
                        "return path",
                result -> {
                    List<Map<String, Object>> maps = Iterators.asList(result);
                    assertEquals(1, maps.size());
    ...
    public void testFilterStartNodeFalseDoesNotFilterStartNodeWhenBelowMinLevel() throws Throwable {
        String query = "MATCH (m:Movie {title: 'The Matrix'}) CALL lds.path.expandConfig(m,{labelFilter:'+Person', minLevel:1, maxLevel:2, filterStartNode:false},datetime()) yield path return count(*) as c";
        TestUtil.testCall(db, query, (row) -> assertEquals(8L, row.get("c")));
    }

    @Test
    public void testOptionalExpandConfigWithNoResultsYieldsNull() {
        String query = "MATCH (m:Movie {title: 'The Matrix'}) CALL lds.path.expandConfig(m,{labelFilter:'+Agent', minLevel:1, maxLevel:2, filterStartNode:false, optional:true}, datetime()) YIELD path RETURN path";
    ...
    public void testFilterStartNodeDefaultsToFalse() throws Throwable {
        ...
        String query = "MATCH (m:Movie {title: 'The Matrix'}) CALL lds.path.expandConfig(m,{labelFilter:'+Person'},datetime()) yield path return count(*) as c";
        TestUtil.testCall(db, query, (row) -> assertEquals(9L, row.get("c")));
    }
    ...
    public void testCompoundLabelMatchesOnlyNodeWithBothLabels() {
        ...
        TestUtil.testResult(db,
                "MATCH (k:Person {name:'Keanu Reeves'}) " +
                        "CALL lds.path.subgraphNodes(k, {relationshipFilter:'ACTED_IN|PRODUCED|DIRECTED', labelFilter:'/Western:Eastwood', uniqueness: 'NODE_GLOBAL'}, datetime()) yield node " +
                        "return node",
                result -> {
    ...
    public void testCompoundLabelWorksInBlacklist() {
        ...
        TestUtil.testResult(db,
                "MATCH (k:Person {name:'Keanu Reeves'}) " +
                        "CALL lds.path.subgraphNodes(k, {relationshipFilter:'ACTED_IN|PRODUCED|DIRECTED', labelFilter:'>Western|-Western:Blacklist', uniqueness: 'NODE_GLOBAL'}, datetime()) yield node " +
                        "return node",
                result -> {
    ...
    public void testRelationshipFilterWorksWithoutTypeOutgoing() {
        TestUtil.testResult(db,
                "MATCH (k:Person {name:'Keanu Reeves'}) " +
                        "CALL lds.path.subgraphNodes(k, {relationshipFilter:'>', labelFilter:'>Movie', uniqueness: 'NODE_GLOBAL'}, datetime()) yield node " +
    ...
    public void testRelationshipFilterWorksWithoutTypeIncoming() {
        TestUtil.testResult(db,
                "MATCH (k:Person {name:'Keanu Reeves'}) " +
                        "CALL lds.path.subgraphNodes(k, {relationshipFilter:'<', labelFilter:'>BigBrother', uniqueness: 'NODE_GLOBAL'}, datetime()) yield node " +
                        "return node",
                result -> {
